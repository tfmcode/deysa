...
$(document).ready(function() {
    ...
    jQuery(".isotope-grid").each(function() {
        ...
        function isotopegrid() {
            ...
            /*PRAGMATICA: si viene con ancla debe abrir solo ese*/
            var hashID = window.location.hash.substring(1);
            if( typeof hashID !== 'undefined' && hashID != '' )
            {
                hashID = "." + hashID;
                filterValue = hashID;
                a.find(".isotope-group a[data-filter='"+filterValue+"']").addClass("active").siblings().removeClass("active");
            }
            /*fin pragmatica*/
            e.isotope({
                getSortData: {
